/* ── Universal Stations v2 — Lecteur vocal d'article ──
   S'attache à #art-body et injecte la pilule "Écouter cet article".
   Synthèse vocale native (gratuite, hors-ligne), surlignage phrase à phrase,
   vitesse réglable. Respecte prefers-reduced-motion pour le défilement. */
(() => {
  const art = document.getElementById('art-body');
  const mount = document.getElementById('voice-mount');
  if (!art || !mount || !('speechSynthesis' in window)) return;
  ...
  play.addEventListener('click', () => {
    if (playing) { playing = false; synth.cancel(); box.classList.remove('playing'); play.textContent = '▶'; return; }
